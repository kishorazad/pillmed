...
export const getSafeImageUrl = (
  ...
  try {
    ...
    const cacheKey = `${safeUrl}|${size || 'original'}|v${CACHE_VERSION}`;
    ...
    const isBlocked = BLOCKED_DOMAINS.some(domain =>
      safeUrl.toLowerCase().includes(domain.toLowerCase())
    );

    if (isBlocked) {
      const fallback = `${DEFAULT_MEDICINE_IMAGE}?v=${CACHE_VERSION}`;
    ...
    // ✅ SAFE SIZE PROCESSING
    if (size && !safeUrl.startsWith('http') && !safeUrl.startsWith('data:')) {
      const dim = IMAGE_SIZES[size];
      const format = getOptimalFormat(safeUrl);

      finalUrl = `${safeUrl}?width=${dim.width}&height=${dim.height}&quality=${dim.quality}&format=${format}&v=${CACHE_VERSION}`;
    }

    // ✅ ADD CACHE VERSION
    if (!safeUrl.startsWith('http') && !safeUrl.includes('?')) {
      finalUrl = `${safeUrl}?v=${CACHE_VERSION}`;
    }
